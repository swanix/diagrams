...
// ============================================================================
// Sistema de progreso de carga para URLs externas y APIs
...
const LoadingProgress = {
  ...
  init() {
    // No necesitamos elementos predefinidos, se crearán dinámicamente
    console.log('[Loading Progress] Sistema inicializado');
  },
  
  // Crear spinner de progreso dinámicamente
  createProgressSpinner() {
    // Remover spinner anterior si existe
    if (this.progressSpinner) {
      this.progressSpinner.remove();
    }
    
    // Crear nuevo spinner de progreso
    this.progressSpinner = document.createElement('div');
    this.progressSpinner.className = 'progress-spinner';
    this.progressSpinner.innerHTML = '<div class="progress-spinner-ring"></div>';
    
    // Insertar directamente en el body
    document.body.appendChild(this.progressSpinner);
  ...
  applyProgressStyles() {
    ...
    style.textContent = `
      .progress-spinner {
        ...
        width: 100vw;
        height: 100vh;
        background-color: var(--canvas-bg, #f6f7f9);
        display: flex;
        align-items: center;
      ...
      .progress-spinner-ring {
        ...
        border-top: 4px solid var(--loading-color, #dadada);
        border-radius: 50%;
      ...
      @keyframes progress-spin {
        ...
        background-color: var(--canvas-bg, #1a1a1a);
              ...
              body.theme-dark .progress-spinner-ring {
        border-color: rgba(255, 255, 255, 0.1);
        border-top-color: var(--loading-color, #00eaff);
      }
    ...
    const existingStyle = document.getElementById('progress-spinner-styles');
    if (existingStyle) {
  ...
  // Mostrar progreso para carga de URLs externas
  showProgress(url, totalSteps = 1) {
    this.isActive = true;
    this.currentStep = 0;
    this.totalSteps = totalSteps;
    this.currentUrl = url;
    
    // Crear y mostrar spinner de progreso
    this.createProgressSpinner();
    
    console.log(`[Loading Progress] Iniciando carga de ${this.getUrlType(url)}`);
  },
  
  // Actualizar progreso (solo para logging, el spinner gira continuamente)
  updateProgress(step, message) {
    if (!this.isActive) return;
    
    this.currentStep = step;
    const percentage = Math.min((step / this.totalSteps) * 100, 100);
  ...
  hideProgress() {
    ...
    console.log('[Loading Progress] Progreso oculto');
  },
  ...
  getUrlType(url) {
    ...
    if (url.includes('google.com/spreadsheets') || url.includes('docs.google.com')) {
      return 'Google Sheets';
    } else if (url.includes('api.') || url.includes('.json') || url.includes('/api/')) {
      return 'API REST';
  ...
  // Progreso para múltiples URLs
  showMultiUrlProgress(urls) {
    this.showProgress(urls[0], urls.length);
  },
  
  // Actualizar para siguiente URL en múltiples
  nextUrl(urlIndex, totalUrls) {
    if (!this.isActive) return;
    
    const url = this.currentUrl;
    const urlType = this.getUrlType(url);
    this.updateProgress(urlIndex + 1, `Cargando ${urlType} (${urlIndex + 1}/${totalUrls})`);
  }
};
...
// Early theme initialization to preserve theme from localStorage
function initializeEarlyTheme() {
  try {
    // Get storage key based on current page
    ...
    if (savedTheme) {
      ...
      if (savedTheme === 'dark' || savedTheme === 'onyx') {
        themeToApply = 'dark';
      } else if (savedTheme === 'light' || savedTheme === 'snow') {
        themeToApply = 'light';
      } else {
        themeToApply = savedTheme; // Use as-is if it's a valid theme name
      }
    }
    ...
    console.log('[Early Theme] Applied theme from localStorage:', themeToApply);
  } catch (error) {
    console.warn('[Early Theme] Error applying early theme:', error);
    // Fallback to default theme
    document.body.classList.add('theme-dark');
  }
}

// Run early theme initialization immediately
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initializeEarlyTheme);
} else {
  initializeEarlyTheme();
}

// Check for ThemeManager loading after a delay and migrate configuration if needed
setTimeout(() => {
  if (window.ThemeManager && !window.ThemeManager.isFallback) {
    console.log('[XDiagrams] ThemeManager detected, migrating configuration...');
    ...
    if (savedTheme) {
      // Migrate to ThemeManager's storage format
      try {
        window.ThemeManager.setTheme(savedTheme);
        console.log('[XDiagrams] Theme configuration migrated to ThemeManager');
      } catch (error) {
        console.warn('[XDiagrams] Error migrating theme configuration:', error);
      }
    }
  }
}, 1000); // Check after 1 second

// Check if ThemeManager is available, if not create fallback
if (!window.ThemeManager) {
  console.log('[XDiagrams] ThemeManager not found, using CSS-only fallback system');
  
  // Create fallback ThemeManager object
  window.ThemeManager = {
    // Mark as fallback for identification
    isFallback: true,
    // Basic theme functions
    setTheme: function(themeId) {
      console.log('[CSS Fallback] Setting theme:', themeId);
      
      // Agregar clase para deshabilitar transiciones solo durante el cambio de tema
      ...
      // Also save theme mode for compatibility
      const themeMode = this.isLightTheme(themeId) ? 'light' : 'dark';
      ...
      // Remover la clase después de un breve delay para restaurar transiciones
      setTimeout(() => {
        document.body.classList.remove('theme-transitioning');
      }, 100); // Aumentado a 100ms para asegurar que el cambio se complete
    },
...
// Get auto image columns configuration
function getAutoImageColumns(diagramConfig = null) {
  // Try diagram-specific configuration first
  if (diagramConfig && diagramConfig.options && diagramConfig.options.autoImageColumns) {
    const columns = diagramConfig.options.autoImageColumns;
    if (Array.isArray(columns)) {
      console.log(`[getAutoImageColumns] Using diagram-specific columns: ${columns.join(', ')}`);
      return columns;
    }
  }
  ...
  // If autoImageColumns is explicitly set in the configuration, use that value
  if (options.hasOwnProperty('autoImageColumns') && Array.isArray(options.autoImageColumns)) {
    console.log(`[getAutoImageColumns] Using global columns: ${options.autoImageColumns.join(', ')}`);
    return options.autoImageColumns;
  }
  
  // Otherwise, return null (no specific columns configured)
  console.log(`[getAutoImageColumns] No auto image columns configured`);
  return null;
}
...
function getThumbnailMode(diagramConfig = null) {
  ...
  if (diagramConfig && diagramConfig.options && diagramConfig.options.thumbnailMode) {
    const mode = diagramConfig.options.thumbnailMode.toLowerCase();
  ...
  if (options.hasOwnProperty('thumbnailMode')) {
    const mode = options.thumbnailMode.toLowerCase();
    if (['default', 'custom', 'none', 'simple'].includes(mode)) {
...
function createClusterTitle(cluster, minX, minY, isFlat = false, diagramName = null, isSingleCluster = false) {
  ...
  // Agregar rectángulo de fondo con variables CSS
  titleGroup.append("rect")
    .attr("class", "cluster-title-bg")
    .attr("rx", 12)
    .attr("ry", 12)
    .style("fill", "var(--cluster-title-bg)")
    .style("stroke", "var(--cluster-stroke)")
    .style("stroke-width", "1px");
  
  // Agregar texto del título con variables CSS
  titleGroup.append("text")
    ...
    .style("fill", "var(--cluster-title-color)")
    .text(clusterTitle);
...
async function updateSidePanelContentCommon(nodeData, diagramConfig = null) {
  ...
  if (idTagElement) {
    ...
    if (nodeIdValue) {
      idTagElement.innerHTML = `<span class="side-panel-id-value">${nodeIdValue}</span>`;
  ...
  // Generate content
  content.innerHTML = await generateSidePanelContent(nodeData, diagramConfig);
...
async function generateSidePanelContent(nodeData, diagramConfig = null) {
  ...
  // Get configuration for hiding empty columns
  const hideEmptyColumns = getHideEmptyColumnsConfiguration(diagramConfig);
  
  // Get auto image columns configuration
  const autoImageColumns = getAutoImageColumns(diagramConfig);
  
  // Collect auto image fields for special team section
  const autoImageFields = [];
  const regularFields = [];
  
  // Process all available fields from the original CSV data
  Object.keys(dataToShow).forEach(key => {
    ...
    // Handle hideEmptyColumns configuration
    if (hideEmptyColumns) {
      if (hideEmptyColumns === "all") {
        // Hide all empty columns
        if (!value) return;
      } else if (Array.isArray(hideEmptyColumns)) {
        // Hide specific columns if they are empty
        if (hideEmptyColumns.includes(key) && !value) return;
      }
    }
    ...
    // Check if this is an auto image column
    ...
    // Separate auto image fields from regular fields
    if (isAutoImageColumn && value && value.trim() !== '') {
      // Split multiple names by comma and create individual team members
  ...
  // Generate team section if there are auto image fields
  if (autoImageFields.length > 0) {
    html += '<div class="side-panel-team-section">';
    html += `<div class="side-panel-team-header">Team <span class="team-counter">${autoImageFields.length}</span></div>`;
    html += '<div class="side-panel-team-members">';
    
    for (const field of autoImageFields) {
      // Normalizar el nombre para buscar la imagen automática
      const normalizedName = field.value.toLowerCase()
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '')
        .replace(/ñ/g, 'n')
        .replace(/\s+/g, '-')
      ...
      // Intentar encontrar la imagen automática usando la misma lógica que resolveNodeImage
      let imageUrl = null;
      ...
      if (autoImagesEnabled !== false) {
        // Precargar imágenes globales si es la primera vez
        if (globalAutoImageCache.size === 0) {
          await preloadGlobalAutoImages();
        }
        
        // Usar la función global findGlobalAutoImageByName para buscar la imagen
        const autoImage = findGlobalAutoImageByName(field.value);
        if (autoImage) {
          imageUrl = autoImage;
        }
      }
...
window.$xDiagrams.loadDiagram = function(input) {
    ...
    // Solo ocultar el topbar si no está ya cargado (carga inicial o refresco)
    const topbar = document.querySelector('.topbar');
    if (topbar && !topbar.classList.contains('loaded')) {
        // El topbar ya está oculto, no hacer nada
    } else if (topbar) {
        // El topbar está visible, mantenerlo visible durante el cambio de diagrama
        // No remover la clase 'loaded'
    }
    ...
    if (originalContainer) {
      ...
      if (jsonThemes) {
        try {
          ...
          container.setAttribute('data-light-theme', 'light');
          container.setAttribute('data-dark-theme', 'dark');
        }
      } else {
        // Fallback to individual attributes
        const lightTheme = originalContainer.getAttribute('data-light-theme') || 'light';
        const darkTheme = originalContainer.getAttribute('data-dark-theme') || 'dark';
...
window.forceDarkTheme = function() {
  ...
  // Update state
  if (window.$xDiagrams.themeState) {
    window.$xDiagrams.themeState.current = 'dark';
  }
...
window.checkThemeOnLoad = function() {
  ...
  const shouldBeDark = specificTheme === 'dark' || globalTheme === 'dark' || themeMode === 'dark';
  const isDarkApplied = bodyClasses.includes('theme-dark');
...
function loadFromCsvUrl(csvUrl, onComplete, retryCount = 0, diagramConfig = null) {
  ...
  // Mostrar progreso para URLs externas
  if (csvUrl && (csvUrl.includes('http') || csvUrl.includes('google.com'))) {
    LoadingProgress.showProgress(csvUrl, 1);
    LoadingProgress.updateProgress(0.5, 'Descargando datos...');
  }
  ...
  Papa.parse(urlWithCacheBuster, {
    ...
    complete: function(results) {
      ...
      // Actualizar progreso
      if (csvUrl && (csvUrl.includes('http') || csvUrl.includes('google.com'))) {
        LoadingProgress.updateProgress(1, 'Procesando datos...');
      }
...
function loadFromMultipleUrls(urls, onComplete, retryCount = 0, diagramConfig = null) {
  ...
  // Mostrar progreso para múltiples URLs
  LoadingProgress.showMultiUrlProgress(urls);
  LoadingProgress.updateProgress(0, `Preparando carga de ${urls.length} hojas...`);
  ...
  async function processUrlsSequentially() {
    for (let index = 0; index < urls.length; index++) {
      ...
      LoadingProgress.nextUrl(index, urls.length);
      LoadingProgress.updateProgress(index + 0.5, `Descargando ${sheetName}...`);
      ...
      try {
        ...
        // Actualizar progreso después de cargar cada hoja
        LoadingProgress.updateProgress(index + 1, `Procesando ${sheetName}...`);
  ...
  function handleMultipleUrlsComplete() {
    ...
    LoadingProgress.updateProgress(urls.length + 0.5, 'Generando diagrama...');
...
function loadFromRestApiWithCache(apiUrl, onComplete, retryCount = 0, diagramConfig = null) {
  ...
  // Mostrar progreso para APIs externas
  LoadingProgress.showProgress(apiUrl, 1);
  LoadingProgress.updateProgress(0.3, 'Verificando caché...');
  ...
  if (shouldCache) {
    ...
    if (cachedData) {
      ...
      LoadingProgress.updateProgress(1, 'Cargando desde caché...');
      ...
      try {
        ...
        LoadingProgress.updateProgress(0.5, 'Cargando desde API...');
      }
    }
  }
  ...
  LoadingProgress.updateProgress(0.6, 'Descargando datos...');
  ...
  fetch(urlWithCacheBuster)
    ...
    .then(jsonData => {
      ...
      LoadingProgress.updateProgress(0.8, 'Procesando datos...');
...
// ============================================================================
// GLOBAL AUTO IMAGE SYSTEM
// ============================================================================

// Cache global para imágenes automáticas
const globalAutoImageCache = new Map();

// Función global para normalizar nombres de imagen
...
async function preloadGlobalAutoImages() {
  ...
  for (const name of commonNames) {
    for (const ext of imageExtensions) {
      ...
      try {
        ...
        if (exists) {
          globalAutoImageCache.set(name, imagePath);
          console.log(`[Global Auto Images] Preloaded: ${imagePath}`);
  ...
  console.log(`[Global Auto Images] Preload complete. Cache contains ${globalAutoImageCache.size} images`);
}

// Función global para buscar imágenes automáticas por nombre
function findGlobalAutoImageByName(name) {
  if (!name || name.trim() === "") return null;
  ...
  if (globalAutoImageCache.has(normalizedName)) {
    const cachedResult = globalAutoImageCache.get(normalizedName);
    console.log(`[Global Auto Images] Cache hit for "${normalizedName}": ${cachedResult}`);
    return cachedResult;
  }
  
  // Si el cache está vacío, no hacer verificación HTTP (evitar requests innecesarios)
  if (globalAutoImageCache.size === 0) {
    console.log(`[Global Auto Images] Cache empty, skipping auto image for "${normalizedName}" to avoid HTTP requests`);
    return null;
  }
  
  // Si no está en cache, no existe la imagen
  console.log(`[Global Auto Images] Cache miss for "${normalizedName}" - image not available`);
  return null;
}
...
async function resolveNodeImage(node, diagramConfig = null) {
  ...
  if (autoImageColumns && autoImageColumns.length > 0) {
    console.log(`[resolveNodeImage] Auto image columns configured: ${autoImageColumns.join(', ')}`);
    
    // Buscar en las columnas configuradas para auto-imágenes
    for (const columnName of autoImageColumns) {
      const columnValue = node[columnName] || (node.data && node.data[columnName]) || "";
      if (columnValue && columnValue.trim() !== "") {
        console.log(`[resolveNodeImage] Checking auto image column "${columnName}" with value: "${columnValue}"`);
        ...
        if (autoImagesEnabled !== false) {
          ...
          const autoImage = findAutoImageByName(columnValue);
          if (autoImage) {
            console.log(`[resolveNodeImage] Auto image found for column "${columnName}" with value "${columnValue}": ${autoImage}`);
            return autoImage;
          }
        }
      }
...
async function createSidePanelThumbnailHtml(nodeData, diagramConfig = null) {
  ...
  // Determinar si es una imagen custom (de la columna Img, automática, o columnas específicas)
  const thumbnailMode = getThumbnailMode(diagramConfig);
  ...
  try {
    ...
    if (tooltip.empty()) {
      tooltip = d3.select('body')
        ...
        .attr('class', 'cluster-tooltip-element');
...
function applyThemeVariablesToTooltip(tooltip) {
  try {
    // Tooltip styles are now handled by CSS classes
    // The tooltip element has the class 'cluster-tooltip-element' which uses CSS variables
    // No need to apply inline styles anymore
    
    console.log('[ClusterTooltip] Theme variables handled by CSS classes');
  } catch (error) {
...
// Function to get hide empty columns configuration from diagram
function getHideEmptyColumnsConfiguration(diagramConfig = null) {
  // Get current diagram configuration
  const currentDiagram = diagramConfig || window.$xDiagrams.currentDiagram;
  
  // Check if hideEmptyColumns is specified in the diagram configuration options
  if (currentDiagram && currentDiagram.options && currentDiagram.options.hideEmptyColumns !== undefined) {
    return currentDiagram.options.hideEmptyColumns;
  }
  
  // Check if hideEmptyColumns is specified in global options
  const globalOptions = getDiagramOptions();
  if (globalOptions && globalOptions.hideEmptyColumns !== undefined) {
    return globalOptions.hideEmptyColumns;
  }
  
  // Default fallback: don't hide any columns
  return false;
}
...
function updateNodeCounter(diagramConfig = null) {
  ...
  if (nodeCountElement && window.$xDiagrams.currentData) {
    ...
    nodeCountElement.style.color = 'var(--ui-focus)';
    
    // After 300ms, transition back to the normal text color
    setTimeout(() => {
      nodeCountElement.style.color = 'var(--control-text)';
    }, 300);
  }
}
