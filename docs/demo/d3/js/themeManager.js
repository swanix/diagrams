...
async function getThemeVariables(themeId, forceReload = false) {
      ...
      const fallbackThemes = {
        ...
        dark: {
          ...
          '--control-focus': '#00eaff'
              ...
              let fallbackVariables = fallbackThemes[themeId] || fallbackThemes.light;
  ...
  if (window.$xDiagrams && window.$xDiagrams.colorBase) {
    ...
    // Esto asegura que no haya conflictos con valores hardcodeados
            if (themeId === 'light') {
          // Para tema light, asegurar que todas las variables usen el color base personalizado
      ...
      themeVariables['--loading-color'] = `hsl(${customColorBase} 40% / 1)`;
          ...
          // Para tema dark, aplicar el color base personalizado manteniendo la oscuridad
...
function getThemeConfiguration() {
  ...
  if (window.getXDiagramsConfiguration) {
    ...
    if (config.themes) {
      return {
        lightTheme: config.themes.light || 'light',
                  darkTheme: config.themes.dark || 'dark'
      };
    }
  }
...
function getThemeConfigurationLegacy() {
  ...
  if (jsonConfig) {
    try {
      ...
      return {
        lightTheme: customConfig.light || 'light',
                  darkTheme: customConfig.dark || 'dark'
      };
    } catch (error) {
  ...
  return {
            lightTheme: container.getAttribute('data-light-theme') || 'light',
          darkTheme: container.getAttribute('data-dark-theme') || 'dark'
  };
}
...
window.reloadThemes = async function() {
  ...
  const storageKey = getStorageKey();
      const currentTheme = localStorage.getItem(storageKey) || 'dark';
